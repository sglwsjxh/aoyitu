# Aoyitu (奥义图) Animation Generator — GUI
#
...
from aoyitu import (AoyituRenderer, _make_gradient, _make_motion_blur,
                    _make_noise, _make_sub_bg)
...
class AoyituGUI:
    '''动画播放器：素材加载 / 参数调优 / 预览播放 / GIF+MP4 导出。'''
    ...
    def _auto_load(self):
        ...
        paths = {
            'clear': os.path.join(SCRIPT_DIR, 'public', 'Sprite', 'qx001.png'),
            'blur': os.path.join(SCRIPT_DIR, 'public', 'Sprite', 'mh001.png'),
            'sub': os.path.join(SCRIPT_DIR, 'public', 'Sprite', 'zm001.png')}
        ...
        # Load default sub_bg
        self.sub_bg_img = self._load_image(os.path.join(SCRIPT_DIR, 'public', 'Sprite', 'zuozhu_000_bg.png'))
        self._load_all()
    ...
    def _build_ui(self):
        ...
        # ---- 素材文件区 ----
        file_frame = tk.LabelFrame(left, text='素材文件', fg=FG, bg=BG, font=FONT)
        ...
        tk.Label(row, text='清晰角色:', fg=FG_DIM, bg=BG, font=FONT_SM, width=10).pack(side=tk.LEFT)
        entry = tk.Entry(row, bg=ENTRY_BG, fg=FG, font=FONT_SM, relief=tk.SOLID, bd=1)
        entry.insert(0, os.path.join('public', 'Sprite', _defaults['clear']))
        entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self._entries['clear'] = entry
        tk.Button(row, text='浏览...', bg=BTN_BG, fg=FG, font=FONT_SM, relief=tk.RAISED, bd=1,
                  command=lambda: self._browse('clear')).pack(side=tk.RIGHT, padx=3)

        subtitle_row = tk.Frame(file_frame, bg=BG)
        subtitle_row.pack(fill=tk.X, pady=2)
        self._sub_cb = tk.Checkbutton(subtitle_row, text='字幕图', variable=self.use_sub_file,
                                      fg=FG_DIM, bg=BG, font=FONT_SM, selectcolor=ENTRY_BG,
                                      activebackground=BG, activeforeground=FG,
                                      command=self._toggle_sub)
        self._sub_cb.pack(side=tk.LEFT)
        self._sub_entry = tk.Entry(subtitle_row, bg=ENTRY_BG, fg=FG_FAINT, font=FONT_SM,
                                   state=tk.DISABLED, relief=tk.SOLID, bd=1)
        self._sub_entry.insert(0, os.path.join('public', 'Sprite', _defaults['sub']))
        self._sub_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self._entries['sub'] = self._sub_entry
        tk.Button(subtitle_row, text='浏览...', bg=BTN_BG, fg=FG, font=FONT_SM, relief=tk.RAISED, bd=1,
                  command=lambda: self._browse('sub')).pack(side=tk.RIGHT)

        self._text_frame = tk.Frame(file_frame, bg=BG)
        ...
        other_text_row = tk.Frame(file_frame, bg=BG)
        self._text_frame2 = other_text_row
        other_text_row.pack(fill=tk.X, pady=2)
        tk.Label(other_text_row, text='其他语言:', fg=FG_DIM, bg=BG, font=FONT_SM, width=10).pack(side=tk.LEFT)
        self._other_text_entry = tk.Entry(other_text_row, bg=ENTRY_BG, fg=FG, font=FONT_SM,
                                          relief=tk.SOLID, bd=1)
        self._other_text_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)

        blur_row = tk.Frame(file_frame, bg=BG)
        blur_row.pack(fill=tk.X, pady=2)
        self._blur_cb = tk.Checkbutton(blur_row, text='模糊图', variable=self.use_blur_file,
                                       fg=FG_DIM, bg=BG, font=FONT_SM, selectcolor=ENTRY_BG,
                                       activebackground=BG, activeforeground=FG,
                                       command=self._toggle_blur)
        self._blur_cb.pack(side=tk.LEFT)
        self._blur_entry = tk.Entry(blur_row, bg=ENTRY_BG, fg=FG_FAINT, font=FONT_SM,
                                    state=tk.DISABLED, relief=tk.SOLID, bd=1)
        self._blur_entry.insert(0, os.path.join('public', 'Sprite', _defaults['blur']))
        self._blur_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self._entries['blur'] = self._blur_entry
        tk.Button(blur_row, text='浏览...', bg=BTN_BG, fg=FG, font=FONT_SM, relief=tk.RAISED, bd=1,
                  command=lambda: self._browse('blur')).pack(side=tk.RIGHT)

        audio_row = tk.Frame(file_frame, bg=BG)
        audio_row.pack(fill=tk.X, pady=2)
        self._audio_cb = tk.Checkbutton(audio_row, text='导入音频', variable=self.use_audio,
                                        fg=FG_DIM, bg=BG, font=FONT_SM, selectcolor=ENTRY_BG,
                                        activebackground=BG, activeforeground=FG,
                                        command=self._toggle_audio)
        self._audio_cb.pack(side=tk.LEFT)
        self._audio_entry = tk.Entry(audio_row, bg=ENTRY_BG, fg=FG_FAINT, font=FONT_SM,
                                     state=tk.DISABLED, relief=tk.SOLID, bd=1)
        self._audio_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        tk.Button(audio_row, text='浏览...', bg=BTN_BG, fg=FG, font=FONT_SM, relief=tk.RAISED, bd=1,
                  command=self._browse_audio).pack(side=tk.RIGHT)
        tk.Button(audio_row, text='+', bg=BTN_BG, fg=FG, font=FONT_SM, relief=tk.RAISED, bd=1, width=2,
                  command=self._add_audio_row).pack(side=tk.RIGHT, padx=1)
        self._audio_extra_container = tk.Frame(file_frame, bg=BG)
        ...
        FONT_BTN = ('SimHei', 10)
        control_bar = tk.Frame(left, bg=BG)
        control_bar.pack(fill=tk.X)
        self._play_btn = tk.Button(control_bar, text='▶ 播放', bg=BTN_GREEN, fg='#fff', font=FONT_BTN,
                                   command=self._toggle_play, width=8)
        self._play_btn.pack(side=tk.LEFT, padx=5)
        tk.Button(control_bar, text='◀◀', bg=BTN_BG, fg=FG, font=FONT_BTN,
        ...
        self._frame_label = tk.Label(control_bar, text='F25/85', fg=FG, bg=BG, font=FONT_BTN)
        ...
        self._frame_slider = tk.Scale(left, from_=0, to=85, orient=tk.HORIZONTAL,
                                      ...
                                      bg=BG_FRAME, fg=FG, troughcolor='#d0d0d0',
                                      highlightthickness=0)
        self._frame_slider.set(25)
        self._frame_slider.pack(fill=tk.X)

        export_bar = tk.Frame(left, bg=BG)
        export_bar.pack(fill=tk.X, pady=5)
        tk.Button(export_bar, text='生成GIF', bg=BTN_RED, fg='#fff', font=('SimHei', 13, 'bold'),
                  command=self._generate).pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=6, padx=(0, 3))
        tk.Button(export_bar, text='生成MP4', bg=BTN_RED, fg='#fff', font=('SimHei', 13, 'bold'),
        ...
        for label, key, default, minv, maxv, step, scale in params:
            row = tk.Frame(scroll_frame, bg=BG_FRAME)
            row.pack(fill=tk.X, pady=1)
            tk.Label(row, text=f'{label}:', fg=FG_DIM, bg=BG_FRAME, font=('SimHei', 10), width=16).pack(side=tk.LEFT)
            var = tk.IntVar(value=default)
            self._params[key] = var
            self._param_scale[key] = scale
            slider = tk.Scale(row, from_=minv, to=maxv, variable=var, orient=tk.HORIZONTAL,
                              bg=BG_FRAME, fg=FG, resolution=step, length=180,
                              troughcolor='#d0d0d0', highlightthickness=0,
                              command=lambda v: self._goto(int(v)))
            slider.pack(side=tk.LEFT)
            val_label = tk.Label(row, text=str(default), fg=FG_DIM, bg=BG_FRAME, font=('SimHei', 9), width=4)
    ...
    def _add_audio_row(self):
        ...
        tk.Button(row_frame, text='浏览...', bg=BTN_BG, fg=FG, font=self._font_sm,
                  ...
                  command=lambda e=entry, v=var: self._browse_audio_entry(e, v)).pack(side=tk.RIGHT)
        tk.Button(row_frame, text='-', bg=BTN_RED, fg='#fff', font=self._font_sm,
                  relief=tk.RAISED, bd=1, width=2,
                  command=lambda e=entry, f=row_frame, v=var: self._remove_audio_row(f, e, v)).pack(side=tk.RIGHT, padx=1)
    ...
    def _load_all(self):
        try:
            ...
            if self.use_sub_file.get():
                ...
                chinese_text = self._chinese_text_entry.get().strip()
                if chinese_text:
                    self.sub_img = None
                    self._chinese_text = chinese_text
                    other_text = self._other_text_entry.get().strip()
                    self._other_text = other_text if other_text else None
                else:
                    self.sub_img = None
                    self._chinese_text = None
                    self._other_text = None
